...
public class BodyLayout extends VerticalLayout {
    ...
    public void loadProjectVersions(Project project) {
        List<ProjectVersion> projectVersionsList = projectVersionService
                .getAllProjectVersionsWithAllVersions(project);
        ...
        projectVersionsSelect.addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                reportsList = reportService.filterByProjectVersion(
                        selectedProject, event.getValue());
                grid.setItems(reportsList);
                distributionBar
                        .setClosed(reportService.getClosedCount(reportsList));
                distributionBar.setUnAssigned(
                        reportService.getUnassignedCount(reportsList));
                distributionBar.setUnResolved(
                        reportService.getUnResolvedCount(reportsList));
    ...
    public void loadReportsGrid() {
        ...
        grid.addColumn(Report::getVersion).setHeader("Version")
                .setKey("version").setSortable(true);
        grid.addColumn(createPriorityRenderer()).setHeader("Priority")
                .setSortable(true).setComparator(Report::getPriority);
    ...
    public void setReportDetailsLayout(
            ReportDetailsLayout reportDetailsLayout) {
